...
### 2.4.2 音声スライス時のエネルギーマップによる $\mathcal{O}(1)$ 最適化

前述の累積和を用いた $\mathcal{O}(N)$ 処理は、音声ファイルロード時のワンタイム特徴抽出には最適である。しかし、BMS生成パイプラインにおいては、同一の `WAV` 音源（ステムデータ）から多数のノーツオブジェクトを切り出す「スライス処理」が数万回規模で並列実行される。

スライスごとに毎回 PCM データを走査・累積和計算すると、CPUのメモリ帯域（I/O）がボトルネックとなりパフォーマンスが低下する。
そこで、`CachedAudioSource` のファイルデコード直後に、AVX2 SIMD ベクトル化を用いて、音声全体のエネルギー分布を一定間隔 $B = 256$ フレーム単位で集約した縮小マップ配列 `long[] EnergyMap` を一度だけ事前計算する設計を導入した。

#### 定数時間 $\mathcal{O}(1)$ 無音トリミング判定
スライス区間の長さ（フレーム数）を $L_{slice}$ とし、対応する開始位置を $startFrame$、終了位置を $endFrame$ とする。
無音トリミングの判定窓幅（$W = 1024$ フレーム）は、エネルギーマップ上のブロック数として $K_W = W / B = 4$ ブロックに換算される。

スライス末尾のトリミング境界を求める際、PCMの波形データ $\mathbf{x}$ には一切アクセスせず、事前計算されたエネルギーマップ $E_{\text{map}}$ 上で逆順に走査を行う。
あるブロックインデックス $i \in [startFrame/B, endFrame/B]$ において、判定窓に相当する隣接 $K_W$ ブロックのエネルギー総和 $E_{\text{window}}$ は次式で与えられる。

$$E_{\text{window}} = \sum_{j=0}^{K_W-1} E_{\text{map}}[i + j] \quad \cdots (2.7)$$

この総和が閾値 $\theta$ 未満である間、ブロック単位でトリミング位置を前進（逆順なので末尾側を削除）させる。
$$E_{\text{window}} < \theta \implies \text{Trim } B \text{ frames}$$

#### 計算量とスループットの劇的改善
- **時間計算量**: スライスごとの波形データの読み込みと計算が完全にゼロ化され、縮小配列上のスキャン（最大でも $L_{slice} / B$ 回のインデックスアクセス）となるため、実質的に $\mathcal{O}(1)$ の定数時間に圧縮された。
- **空間およびメモリ転送コスト**: スライス用スレッドが巨大な PCM 配列を繰り返し舐める必要がなくなり、CPU キャッシュ内で完全に処理が完結する。
- **結果**: 100,000ノーツ規模の処理において、スライス・無音検出フェーズの処理速度が劇的に向上した。
