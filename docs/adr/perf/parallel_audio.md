...
- **パラメータオブジェクトの導入**: 内部処理（`CompareGroup` / `PerformSortAndSweep` / `CompareFilePair` / `MarkSelf`）で多数のパラメータ（特に複数の `ref` 変数）が引き回されていたシグネチャを整理し、ラン全体の設定・状態をカプセル化する `ComparisonContext` と、グループごとの結果統計を保持する `GroupComparisonStats` に分割・カプセル化。可読性と保守性を向上させました。
- **走査対象の事前絞り込みによるホットパス最適化**: 従来、対角線走査の二重ループ内部で毎回行っていたインデックス範囲チェック（`_startPoint`/`_endPoint`）を排除するため、走査開始前に範囲内の要素だけを `ArrayPool<AudioEntry>` で事前抽出（$O(N)$）し、二重ループの走査サイズを $\frac{N(N-1)}{2}$ から $\frac{M(M-1)}{2}$ （ここで $N$ はグループ内の全要素数、$M$ はそのうち範囲内にある要素数で $M \le N$）へと大幅に削減。また、統合済み（`IsMapped`）である場合の早期スキップをループ先頭で行うことで、ホットパスにおける不要な浮動小数点計算や条件分岐のオーバーヘッドを極限まで低減しました。
