...
「手動で重複した音源の定義を削減するの、死ぬほどめんどいでしょ？だったらプログラム作ろうぜ」というのが狂気じみているかもしれませんが、 BMS Part Tuner を開発した私の原動力でした。
...
graph TD
    subgraph naive ["❌ 素朴な方法（全員書き換え）"]
        ...
        社員B2[社員4] --> 会社B
        会社B -.-> |全員の名札を書き換え| 会社A
        note1[合併のたびに全員分書き換えてフリーズ]
    end
```

```mermaid
graph TD
    subgraph uf ["✨ Union-Find（社長だけ繋ぎ替え）"]
        direction TB
        社員1[社員1] --> 社長A((社長A))
        社員2[社員2] --> 社長A
        社員3[社員3] --> 社長B((社長B))
