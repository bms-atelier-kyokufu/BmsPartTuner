...
namespace BmsAtelierKyokufu.BmsPartTuner.Generators
{
    ...
    public class SimdBatchUnrollGenerator : IIncrementalGenerator
    {
        ...
        private static string GenerateMethodSource(string ns, string cls, string method, int unrollFactor, string logicType, bool isStatic)
        {
            ...
            string methodBody = logicType switch
            {
                "PearsonNormalized" => GeneratePearsonNormalizedBody(method, unrollFactor),
                "PearsonStats" => GeneratePearsonStatsBody(method, unrollFactor),
                "DotProduct" => GenerateDotProductBody(method, unrollFactor),
                _ => ""
            };
        ...
        private static string GeneratePearsonNormalizedBody(string method, int unrollFactor)
        {
            var initDots = string.Join("\n", Enumerable.Range(0, unrollFactor).Select(k =>
            $"Vector<float> dot{k} = Vector<float>.Zero;"));
        ...
        private static string GeneratePearsonStatsBody(string method, int unrollFactor)
        {
...
"""));
            ...
            return $$"""
        private static partial void {{method}}(ReadOnlySpan<float> wav1, ReadOnlySpan<float> wav2, int vectorizedLength, int vectorSize, out PearsonAccumulator accumulator)
        {
            int unrollStep = vectorSize * {{unrollFactor}};
...
{{sumVars}}
            ...
            accumulator = new PearsonAccumulator
            {
...
""";
        ...
        private static string GenerateDotProductBody(string method, int unrollFactor)
        {
            var initDots = string.Join("\n", Enumerable.Range(0, unrollFactor).Select(k => $"            Vector<float> dot{k} = Vector<float>.Zero;"));

            var unrolledLoopBody = string.Join("\n", Enumerable.Range(0, unrollFactor).Select(k =>
                $"                dot{k} += new Vector<float>(span1.Slice(i + {k} * vectorSize, vectorSize)) * new Vector<float>(span2.Slice(i + {k} * vectorSize, vectorSize));"));

            var sumDots = string.Join("\n", Enumerable.Range(0, unrollFactor).Select(k => $"            dotTotal += dot{k};"));

            return $$"""
        private static partial float {{method}}(ReadOnlySpan<float> span1, ReadOnlySpan<float> span2)
        {
            if (span1.Length != span2.Length || span1.Length == 0) return 0.0F;
...
{{sumDots}}
            ...
            for (int i = remainderStart; i < vectorizedLength2; i += vectorSize)
            {
                dotProduct += Vector.Dot(new Vector<float>(span1.Slice(i, vectorSize)) * new Vector<float>(span2.Slice(i, vectorSize)), new Vector<float>(1.0f));
            }
