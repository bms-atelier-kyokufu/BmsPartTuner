...
internal sealed class LoadValidFilesStep : IAsyncOptimizationStep
{
    ...
    public Task ExecuteAsync(OptimizationSimulationContext context)
    {
        ...
        if (actualEndDefinition == 0)
        {
            ...
            PerformanceDebugLogger.WriteDebug(Name, $"Auto-detected end definition: {actualEndDefinition}");
        }
        ...
        PerformanceDebugLogger.WriteDebug(Name, $"Valid files loaded: {originalCount}, Actual range: {context.StartDefinition}-{context.EndDefinition}");
...
internal sealed class PreloadAudioCacheStep : IAsyncOptimizationStep
{
    ...
    public Task ExecuteAsync(OptimizationSimulationContext context)
    {
        ...
        {
            PerformanceDebugLogger.WriteDebug(Name, $"WARNING: {failedFiles.Count} files failed to load");
            foreach (string? file in failedFiles.Take(5))
            {
                PerformanceDebugLogger.WriteDebug(Name, $"  - {Path.GetFileName(file)}");
            }
            if (failedFiles.Count > 5)
            {
                PerformanceDebugLogger.WriteDebug(Name, $"  ... and {failedFiles.Count - 5} more");
            }
        }
...
internal sealed class RunParallelSimulationStep : IAsyncOptimizationStep
{
    ...
    public async Task ExecuteAsync(OptimizationSimulationContext context)
    {
        ...
        if (context.SimulationData.Count > 0)
        {
            ...
            PerformanceDebugLogger.WriteDebug(Name, $"File count range in results: {minCount} - {maxCount}");
        }
...
internal sealed class FindOptimalThresholdsStep : IAsyncOptimizationStep
{
    ...
    public Task ExecuteAsync(OptimizationSimulationContext context)
    {
        ...
        {
            PerformanceDebugLogger.WriteDebug(Name, "Base62: Using Base36 threshold as fallback (no better option found)");
            base62Threshold = base36Threshold;
            base62Count = base36Count;
        }

        PerformanceDebugLogger.WriteDebug(Name, $"Base36: Threshold={base36Threshold:F2}, Count={base36Count}");
        PerformanceDebugLogger.WriteDebug(Name, $"Base62: Threshold={base62Threshold:F2}, Count={base62Count}");
        ...
        if (context.FailedFiles.Count > 0)
        {
            ...
            PerformanceDebugLogger.WriteDebug(Name, $"Added warning to result: {warningMessage}");
        }
