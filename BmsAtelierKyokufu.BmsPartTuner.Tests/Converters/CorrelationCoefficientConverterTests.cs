...
/// <para>【テスト対象】</para>
/// <list type="bullet">
/// <item><description>Convert: 内部値(0.00-1.00) → 表示値(0-100)</description></item>
/// <item><description>ConvertBack: 表示値(0-100) → 内部値(0.00-1.00)</description></item>
/// </list>
/// <para>【テスト設計方針】</para>
/// <list type="bullet">
/// <item><description>境界値分析: 0, 1, 100 などの境界値</description></item>
/// <item><description>同値分割: 正常値、異常値、エッジケース</description></item>
...
public class CorrelationCoefficientConverterTests
{
    ...
    private void AssertConvert(object? input, string expected, CultureInfo? culture = null)
    {
        var result = _converter.Convert(input!, typeof(string), null!, culture ?? _culture);
        Assert.Equal(expected, result);
    }

    private void AssertConvertBack(object? input, string expected)
    {
        var result = _converter.ConvertBack(input!, typeof(string), null!, _culture);
        Assert.Equal(expected, result);
    }

    private static string GetExpectedDefault()
    {
        return AppConstants.Threshold.Default.ToString("F2");
    }
    ...
    public void ConvertBack_NonStringValue_ReturnsDefaultValue()
    {
        ...
        AssertConvertBack(nonStringValue, GetExpectedDefault());
    }
    ...
    public void Convert_DifferentCultures_ProducesSameResult(string cultureName)
    {
        ...
        AssertConvert(0.95f, "95", culture);
    }
    ...
    {
        const string input = "0.123456789";  // 非常に長い小数
